...
public class BGABindingRecyclerViewAdapter<M, B extends ViewDataBinding> extends RecyclerView.Adapter<BGABindingViewHolder<B>> {
    ...
    public void onBindViewHolder(BGABindingViewHolder<B> viewHolder, int position) {
        ...
        binding.setVariable(com.qiwu.ui.BR.viewHolder, viewHolder);
        binding.setVariable(com.qiwu.ui.BR.model, model);
        binding.setVariable(com.qiwu.ui.BR.itemEventHandler, mItemEventHandler);
    ...
    public void addNewData(List<M> data) {
        if (BGAAdapterUtil.isListNotEmpty(data)) {
            mData.addAll(0, data);
            notifyItemRangeInsertedWrapper(0, mData.size());
        }
    }
