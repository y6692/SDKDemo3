...
public class RecyclerViewPager extends RecyclerView {
    ...
    public int getCurrentPosition() {
        int curPosition;
        if (getLayoutManager().canScrollHorizontally()) {
            curPosition = UIUtils.getCenterXChildPosition(this);
        } else {
            curPosition = UIUtils.getCenterYChildPosition(this);
        }
        if (curPosition < 0) {
    ...
    protected void adjustPositionX(int velocityX) {
        ...
        if (childCount > 0) {
            int curPosition = UIUtils.getCenterXChildPosition(this);
            ...
            if (targetPosition == curPosition
                ...
                View centerXChild = UIUtils.getCenterXChild(this);
    ...
    protected void adjustPositionY(int velocityY) {
        ...
        int childCount = getChildCount();
        if (childCount > 0) {
            int curPosition = UIUtils.getCenterYChildPosition(this);
            ...
            if (targetPosition == curPosition
                    && (!mSinglePageFling || mPositionOnTouchDown == curPosition)) {
                View centerYChild = UIUtils.getCenterYChild(this);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN && getLayoutManager() != null) {
            mPositionOnTouchDown = getLayoutManager().canScrollHorizontally()
                    ? UIUtils.getCenterXChildPosition(this)
                    : UIUtils.getCenterYChildPosition(this);
            if (DEBUG) {
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == SCROLL_STATE_DRAGGING) {
            mNeedAdjust = true;
            mCurView = getLayoutManager().canScrollHorizontally() ? UIUtils.getCenterXChild(this) :
                    UIUtils.getCenterYChild(this);
            if (mCurView != null) {
            ...
            if (mNeedAdjust) {
                int targetPosition = getLayoutManager().canScrollHorizontally() ? UIUtils.getCenterXChildPosition(this) :
                        UIUtils.getCenterYChildPosition(this);
                if (mCurView != null) {
