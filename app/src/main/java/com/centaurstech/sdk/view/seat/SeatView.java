...
public class SeatView extends View {
    ...
    private List<String> mSeatNumber = new ArrayList<>();
    ...
    int bestTop;
    int bestBottom;
    private SeatBestViewPainter mSeatBestViewPainter;
    private Paint mBestViewPaint = new Paint();
    private RectF mBestViewRectF = new RectF();
    ...
    private void init(
        ...
        if (mSeatNormal != null) {
            ...
            this.mSeatWidth = Utils.dp2px(context, 60);
...
//        if (null != bestRows && bestRows.length > 0) {
//            minRows = Integer.parseInt(bestRows[0]);
//        }
//        if (null != bestRows && bestRows.length > 1) {
//            maxRows = Integer.parseInt(bestRows[1]);
//        }
//
//        if (null != bestCols && bestCols.length > 0) {
//            minCols = Integer.parseInt(bestCols[0]);
//        }
//        if (null != bestCols && bestCols.length > 1) {
//            maxCols = Integer.parseInt(bestCols[1]);
//        }
...
//            if (seat.point.x == minRows) {
//                bestTop = (int) (mDrawStartY + seatDrawHeight * (minRows - 1));
//            }
//            if (seat.point.x == maxRows) {
//                bestBottom = (int) (bestTop + seatDrawHeight);
//            }
//
//            if (seat.point.y == minCols) {
//                bestLeft = (int) (mDrawStartX + seatDrawWidth * (minCols - 1));
//            }
//
//            if (seat.point.y == maxCols) {
//                bestRight = (int) (bestLeft + seatDrawWidth);
//            }
...
//        if (bestRows != null || bestCols != null) {
//            drawViewingArea(canvas, bestLeft, bestTop, bestRight, bestBottom);
//        }
        ...
        if (lineX > 0 && lineY > 0 && mShowCenterLine) {
            ...
            float startY;
            if (seatNotFill) {
                startY = lineStartY + seatDrawHeight * 2 * (minGraphRow - 1);
            } else {
                startY = 0;
            }
     ...
     * 绘制最佳观影区
     */
    private void drawViewingArea(Canvas canvas, int left, int top, int right, int bottom) {
...
//        mBestViewPaint.setPathEffect(new DashPathEffect(new float[]{4, 4}, 0));
        mBestViewRectF.set(left, top, right, bottom);
        canvas.drawRect(mBestViewRectF, mBestViewPaint);
    }
...
//                isMatchRegular = checkSeatRegular(seat, true);
     ...
     * @param seats 座位列表
     */
    public void setSeatData(List<SeatData> seats, List<String> mSeats) {
...
//                mSeatNo[i - 1] = count > 0 ? mSeats.get(i - 1) : "";
//            }
//        }
        if (mMaxRow > 0 && rowMap.size() > 0) {
            mSeatNo = new String[mMaxRow];
            for (int i = 0; i < mSeats.size(); i++) {
                mSeatNo[i] = mSeats.get(i);
    ...
    private void initSeatScale(int width, int height) {
        ...
        mMinScale = Math.min(scaleX, scaleY);
    }
    ...
    public void setBestRows(String[] bestRows, String[] bestCols) {
        this.bestCols = bestCols;
